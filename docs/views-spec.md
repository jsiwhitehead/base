...
- Outline is a hierarchical lines-of-text editor: value nodes are lines, and items define indentation levels.
- Nodes are either `item` nodes or value nodes (plain `value` or connected `conn:*`).
- Navigation is hierarchical and depth-first across visible nodes.
- Editing stays inline in the outline.
- Traversal follows visible node stops.
- `Enter` on node selection goes inward; structural insertion uses modified Enter shortcuts.
...
```text
.ui-body.ui-outline
  .ui-frame.ui-outline-child                   (target: NODE_TARGET)
...
- `.ui-frame.ui-outline-child` instances MUST stay stable per visible child node.
...
- When an item has zero children, Outline MUST render a body-only empty state.
- Empty-state UI for an empty item MUST NOT be a location surface and MUST NOT add edit targets.
...
- For the current outline node, `.ui-frame` and `.ui-header` are outside `.ui-body.ui-outline` and are outer-view-owned.
- For each visible child node, outline renders a `.ui-frame.ui-outline-child` that hosts the child's optional `.ui-header` subtree and mounted child body subtree.

### Location surfaces and targets

Outline surfaces:

- `NODE_TARGET` on the node's `.ui-frame`
...
- Traversal follows visible node stops.
- Items participate in navigation but do not contribute a stop directly.
...
- Connected Outline nodes are atomic traversal stops at `NODE_TARGET`.
- Non-outline child views are atomic traversal stops at `NODE_TARGET`.
...
- Node has a non-empty label.
- Node has connected fields.
...
- Gutter/rail region (left of `--outline-indent`) keeps frame `NODE_TARGET` behavior. `span.ui-outline-gutter` MUST call `e.preventDefault()` on `pointerdown` (structural chrome requirement for `contenteditable` views; see `docs/content-editable.md`).
...
When an item is empty and node selection is on that item (`NODE_TARGET`):

- `TYPE` with any other character MUST convert the item to `value` with `""`, focus `content:text`, and type that character (replace behavior).
- `ENTER` MUST convert the item to `value` with `""` and focus `content:text`.
...
Node selection uses sibling-only vertical navigation. At boundaries (no parent, no child, no previous/next sibling), NAV is a no-op.

| NAV   | From node selection            |
| ----- | ------------------------------ |
| Up    | Previous sibling               |
| Down  | Next sibling                   |
| Left  | Parent node                    |
...
Tab changes structure via **in-place body transforms** (node identity stays in place).

- **Tab (indent)**: the focused node remains in place and keeps its **label** and **view**. Outline inserts a single child node and copies the focused node's **body content** into that child. The focused node's label remains on the parent node. If the focused node is `connected`, indent is a no-op.
- **Shift+Tab (outdent)**: the focused node's parent remains in place and keeps its **label** and **view**. Outline replaces the parent's **body content** with the focused child's body content, then removes the focused child and all its siblings under that parent. The focused child's label, if any, is discarded. If the parent is `connected`, outdent is a no-op.
- Preserve the current target and caret when possible. Otherwise, no-op.

#### Edit traversal scope

Traversal is unified across visible Outline nodes in depth-first order, with at most one stop per node.

- Plain value nodes contribute one `content:text` stop.
- Connected Outline nodes and non-outline child views contribute one atomic `NODE_TARGET` stop.
- Header controls do not participate.
- Outline items do not contribute a stop directly; they contribute the stops of their visible descendants.

#### Inter-node edge behavior
...
When the adjacent stop is a connected Outline node or a non-outline embedded child view, traversal lands on that row's `NODE_TARGET`. Subsequent arrow navigation from that stop uses ordinary Outline node-selection geometry until the user explicitly goes inward.

For `ArrowUp` / `ArrowDown`, native text movement remains in effect while the caret can still move within the current `content:text` block. At the first or last visual line boundary, vertical movement follows the same stop model: plain-value stops re-enter `content:text`, while connected and embedded stops land on `NODE_TARGET`.
Sticky column is preserved only within the same continuous vertical `content:text` session.

Header text-editing keys stay local/native. `Enter` commits and exits to same-node selection. `Escape` cancels and exits to same-node selection. For `conn:*`, `Tab` / `Shift+Tab` move within canonical shared-header field order when another field exists; otherwise they commit and no-op.

#### Enter behavior

- `Enter` from a plain value `content:text` target splits at caret and focuses the new sibling node's `content:text` target at caret `0`. This applies only to plain value `content:text` targets, never to `conn:*`.
- `Shift+Enter` from a plain value `content:text` target inserts a newline in place and keeps edit focus on the same node.
- `Mod+Enter` inserts a new plain sibling after the focused node and focuses its `content:text` target at caret `0`.
- `Mod+Shift+Enter` inserts a new plain node after the parent only when the focused node is the last child of a non-root item; otherwise it is a no-op. Focus moves to the new node's `content:text` target at caret `0`.

These structural intents are always handled by Outline, even when focus is on an embedded child view's `content:*` target.

#### Node-selection Enter behavior

`Enter` from node selection goes inward:

- Plain value or embedded leaf node: focus the primary edit target if present.
- Non-empty item: focus the first child at `NODE_TARGET`.
- Empty item: create the first child and focus its `content:text` target with caret at start.

#### DELETE policy

- Outline handles DELETE with remove semantics (not clear-in-place).
- `NODE_TARGET` DELETE MUST remove the selected subtree.
- Delete at boundary from a `conn:*` target is a no-op.
- Delete at boundary from a `content:text` target is target-specific:
  If the current text is non-empty, boundary delete checks the adjacent edit stop in the delete direction. If that stop is a plain value node, Outline joins the two values at the boundary point and places the caret at the join boundary in the surviving node. Otherwise, no-op.
  If the current text is empty, that DELETE intent removes the node and moves to the adjacent edit stop in the unified traversal when one exists. Backward moves to the previous stop with caret at end; forward moves to the next stop with caret at start.
- When the contenteditable selection spans multiple plain value nodes within the same parent, delete/backspace merges the start node's text up to the selection start with the end node's text from the selection end, removing all spanned nodes between them. The caret lands at the merge point in the surviving start node. Constrained to same-parent siblings; cross-parent ranges are a no-op.
...
- Structural removals triggered by DELETE (`NODE_TARGET`, block selection, empty-`content:text`, and join removal of the absorbed neighbor) MUST prune newly-empty ancestor items in the same commit.
- Pruning stops at `rootId`, readonly ancestors, non-item ancestors, or when an ancestor remains non-empty.
...
- `out` (Shift+Tab): unwrap the parent item by moving `G(children...)` into `G`'s parent at `G`'s original index. `G.label` is discarded.
...
- No-op: do nothing when the required structure is missing, when promotion preconditions fail, when content is not plain `value`/`item`, or when any required node is `connected` or `readonly`.

### Edge cases and invariants

- Prune invariant: After any Outline structural edit, Outline MUST NOT leave any newly-empty items in the edited ancestry; newly-empty items MUST be removed immediately.
- Root normalization: if a structural removal leaves the current Outline `rootId` as an empty item, Outline MUST clear that `rootId` to blank.
- Outline MAY still encounter pre-existing empty items (for example, from other views); these are handled by the placeholder rule.

### Styling notes

Outline-local styling:

- One left rail segment per node depth.
- Indentation based on `(rail + pad)` per depth level.
- Header capsule aligns to node rail start.
...
- Table node children represent rows.
- Row node children represent cells.
...
- Table distinguishes **node selection** from **cell edit focus**.
...
.ui-body.ui-table
  .ui-table-inner
    .ui-table-header
      ...
      ...
    .ui-table-body
      .ui-frame.ui-table-row                   (row target: NODE_TARGET)
        .ui-table-cell.ui-table-first
          [.ui-header subtree]                 (row header; targets: label, conn:*)
        .ui-frame.ui-table-cell                (cell target: NODE_TARGET)
...
- Dropping a node on a cell's top/bottom edge MUST insert the dragged node as a row before/after the target row.

Notes:

- For the table node itself, `NODE_TARGET` is on the table's `.ui-frame` outside `.ui-body.ui-table`.
- Row/cell node selection in table body MUST be implemented as Core location surfaces on `.ui-frame.ui-table-row` / `.ui-frame.ui-table-cell`, not raw DOM focus.

### Location surfaces and targets

Table location and target modes:

- **Table frame node selection**:
  - `NODE_TARGET` on the table's `.ui-frame`

- **Row node selection**:
  - location refers to a row node
  - `NODE_TARGET`

- **Cell node selection**:
  - location refers to a cell node
  - `NODE_TARGET`
...
- Table MUST distinguish node selection from `content:text` edit focus.
- Table MUST keep embedded outline editing local unless the embedded editor explicitly yields structural behavior.
- Directional `NAV` and `DELETE` are local node-selection operations in table mode. `Tab` is handled locally by the table view.
- Row/cell node selection MUST be implemented as Core location surfaces backed by stable `.ui-table-row` and `.ui-table-cell` wrappers, not raw DOM focus.
...
| NAV   | From row `NODE_TARGET` | From cell `NODE_TARGET`                             |
| ----- | ---------------------- | --------------------------------------------------- |
| Up    | Previous row           | Same column, previous row                           |
| Down  | Next row               | Same column, next row                               |
| Left  | Exit to table node     | Previous cell (column 0 exits to row `NODE_TARGET`) |
| Right | First cell             | Next cell                                           |

#### Tab action

Move right (Tab) or left (Shift+Tab) across cells, wrapping across rows, from table-owned node selection. From row `NODE_TARGET`, Tab enters the first cell and Shift+Tab is a no-op. Tab from embedded edit focus remains local to the embedded editor unless that editor explicitly yields structural behavior.

#### Edit traversal scope

Scoped to a single node. The traversal moves through that node's edit targets only.
...
Table-owned cross-node operations (node-selection `NAV`, node-selection `DELETE`, and local node-level `Tab`) land at node selection on the destination. Edit is entered explicitly via `ENTER` or `TYPE`, or remains inside the embedded cell view when that view keeps local ownership.

#### DELETE policy

Rows use remove with a last-row special case:

- If the table has more than one row, remove the row. After removing a row, focus the next row at row `NODE_TARGET`, then previous row `NODE_TARGET`, then table `NODE_TARGET`.
- If the row is the last remaining row, remove the whole table node.

Cells use clear — clear the cell to blank and stay on the same cell at node selection.
...
- Slider uses:
  - `NODE_TARGET` on the slider frame.
  - `content:slider` on the `<input type="range">`.
- Keyboard behavior is primarily native to the range input.

### View-specific behaviors

Slider is only ever used as a node view, never as an outer view. Its body is a range input rather than a text field.
...
- `setValue` SHOULD commit only when node is an editable plain value node and value is finite.

### Edge cases and invariants

Rules:

- Slider MUST only edit plain value nodes.
...
- Slider input MUST be disabled when node is not editable.
...
- Meaning of `NODE_TARGET` in that view context.
- Edit-target behavior from node selection.
