...
Registers a binding between focus state and concrete elements.

- `elementFor(target)` is queried when focus changes.
- `caret` (if provided) allows Core to position text cursors.
- Returns a cleanup function that must be called when the binding is no longer valid.
- For a given `(container, item)` focus, only one focus binding should be active at a time.
