...
function createLibrary(): Record<string, DataSignal> {
  return {
    ...
    mod: typedFn([reqNum, optNum(1)], (d, m) =>
      createLiteral(((d % m) + m) % m)
    ),
    ...
    filter: typedFn([reqBlock, reqFn], (source, predNode) =>
      blockFilter(source, (value, id) =>
        boolExpect(predNode.fn(value, id).get())
      )
    ),
...
export function withLibrary(
  ...
  markScopeCaseInsensitive(createBlockSignal(createLibrary(), [root]));
  return root;
}
